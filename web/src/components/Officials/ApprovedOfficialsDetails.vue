  ...
  setup(props, ctx) {
    ...
    const approve = () => {
      useAlert().confirmBeforeAction(
        'Are you sure? This Official would be denied access to the web app',
        disableOfficial,
        id.value
      )
    }
    const disableOfficial = (id: any) => {
      useApprovedOfficials().disable(id)
      close()
    }
    ...
    return { fullname, phone, email, role, state, local_govt, approve, close }
  }
})
</script>
...
<template>
  <div
    v-if="show && official"
    class="fixed top-0 bg-nadis-fade h-screen"
