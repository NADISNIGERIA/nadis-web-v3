...
import PendingOfficialDetails from './PendingOfficialDetails.vue'

export default defineComponent({
  components: { PendingOfficialDetails },
  props: {
    full: Boolean
  },
  setup() {
    ...
    const pending_officials = computed(() => usePendingOfficials().pending_officials) as any

    // Filtered and sorted officials
    const filteredOfficials = computed(() => {
      if (!pending_officials.value) return []
      
      let filtered = pending_officials.value.filter((official: any) => {
    ...
    const role = (val: any) => {
      if (val == 'Federal') {
        return 'Will Have Access to All'
      } else if (val == 'State') {
        return 'Will Have Access to Own State'
      } else {
        return 'Will Have Access to Own LGA'
      }
    }
    ...
    <div class="mb-6">
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div class="flex items-center space-x-4">
          <h2 class="text-2xl font-semibold text-nadis-black">Pending Officials</h2>
    ...
    <div class="space-y-6">
      ...
      <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
                ...
                >
                  <div class="flex items-center space-x-1">
                    ...
                    <svg
                      class="w-4 h-4 transition-transform"
                      :class="{
                        'text-yellow-600': sortField === 'role',
        ...
        <div v-else class="text-center py-12">
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900">No pending officials found</h3>
          <p class="mt-1 text-sm text-gray-500">
            {{ searchQuery ? 'Try adjusting your search criteria.' : 'All officials have been processed or no officials are currently pending approval.' }}
          </p>
        </div>
      </div>
      ...
      <PendingOfficialDetails :show="show" :official="official" @close="openModal" />
    </div>
  </div>
</template>
