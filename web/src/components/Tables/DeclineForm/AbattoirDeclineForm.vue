...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const text_lenght = ref(0)
    const total_chars = ref(150)
    const disable = ref(false)

    watch(form_report, () => {
      text_lenght.value = form_report.value.length
      if (text_lenght.value > total_chars.value) {
        disable.value = true
      } else if (text_lenght.value <= total_chars.value) {
        disable.value = false
      }
    })
    ...
    const validation = () => {
      if (props.doc_id != undefined && form_report.value != '') {
        ...
        useAlert().exposeAlert(4000, 'info', 'Fill All Fields')
      }
    }

    return { close, addForm, text_lenght, total_chars, form_report, disable }
  }
})
</script>
  ...
  >
    <div class="absolute top-1/10 md:top-1/4 w-full">
      <div class="relative mx-auto w-9/10 lg:w-2/4 px-12 py-16 bg-white rounded overflow-y-scroll">
        ...
        <div>
          <p class="text-3xl">Abattoir Decline Form</p>
          ...
          <div class="pt-10">
            <div class="space-y-2">
              <div
                class="float-right"
                :class="{
                  'text-red-600': text_lenght > total_chars,
                  'text-green-600': text_lenght == total_chars
                }"
              >
                ( <span> {{ text_lenght }}</span> of <span>{{ total_chars }} </span>)
              </div>
              <input-text-area
                type="text"
