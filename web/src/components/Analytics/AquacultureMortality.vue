...
export default defineComponent({
  setup(props, ctx) {
    ...
    const aquaculture_mortality = computed(() => useMortalityRate().aquaculture_mortality)
    ...
    const bar_array = computed(() => {
      ...
      array.push(add_increment)
      ctx.emit('bar-array', array)

      return array
    })

    watch(aquaculture_mortality, () => {
      callMonth()
    })
    watch(mortality, () => {
    ...
    const callMonth = () => {
      ...
      for (let index = 0; index < number_of_months.value; index++) {
        const values = useMortalityRate().aquaculture_number_month(index)
        let added = 0
        values.forEach((value) => {
          added = added + parseInt(value.passive_surveillance.mortality_for_month)
          if (parseInt(value.passive_surveillance.mortality_for_month) > largest.value) {
            largest.value = parseInt(value.passive_surveillance.mortality_for_month)
          }
        })
        mortality.value.push(added)
        ctx.emit('aquaculture-mortality', mortality.value)
      }
    }
    const callLength = (val: any) => {
      ...
      for (let index = 0; index < 5; index++) {
        ...
        ctx.emit('bar-lengths', bar_lengths.value)
      }
    }
    const monthCalculator = () => {
      for (let index = number_of_months.value - 1; index > -1; index--) {
        ...
        ctx.emit('rate-months', rate_months.value)
      }
    }
...
<template>
  <div class="w-full lg:w-1/2 py-8 text-left">
    <div class="pl-4 text-black">
      <div class="text-xl">Aquaculture Mortality Rate (Last 5 Months)</div>
    </div>
    <div class="my-6 mr-2 pl-20 pr-5">
      ...
      <div class="relative">
        <div class="absolute text-xs -top-20 text-gray-500 w-full">
          <div class="grid grid-cols-5">
            <div
              class="w-8 rounded-sm bg-gradient-to-t from-blue-100 via-blue-300 to-blue-500"
