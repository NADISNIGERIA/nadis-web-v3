      ...
      try {
        for (const docId of docIds) {
          try {
            ...
            await updateDoc(doc(fb.db, 'suspicion_reports', docId), updates)
            results.success.push(docId)

            emitReportStatsUpdate(REPORT_TYPES.SUSPICION, newStatus === 'approved' ? 'approve' : newStatus === 'in_progress' ? 'in_progress' : 'pending', docId, this)
          } catch (error) {
