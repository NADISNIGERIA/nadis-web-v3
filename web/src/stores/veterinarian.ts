...
export const useVeterinarian = defineStore('veterinarian', {
  state: () => ({
    veterinarian: [],
  ...
  actions: {
    ...
    async show_in_progress_a_state(state: any) {
      ...
      this.veterinarian = value.sort((a: any, b: any) => b.created_at - a.created_at)
    },

    async getVeterinarianCount(values: any) {
      try {
        const state = values.state
        let queryRef = collection(fb.db, 'veterinarian_reports')
        ...
        queryRef = query(collection(fb.db, 'veterinarian_reports'), ...queryConstraints)

        const snapshot = await getCountFromServer(queryRef)
        return snapshot.data().count
      } catch (error) {
        console.error('Error getting veterinarian count:', error)
        return 0
      }
    },
    ...
    async show_in_progress_all_states() {
      const docs = await getDocs(
        query(collection(fb.db, 'veterinarian_reports'), where('finished', '==', false))
      )
      const value = [] as any
      this.reporter_state = []
      docs.forEach((doc) => {
    ...
    async getVeterinarian(values: any, isNextPage = false, pageSize = 20) {
      try {
        this.loading = true
        const state = values.state
    ...
    async loadPagesSequentially(values: any, targetPage: number, pageSize: number, sort: boolean, state: string) {
      ...
      for (let page = 1; page <= targetPage; page++) {
        ...
        if (page === 1) {
          ...
          try {
            this.pagination.totalCount = await this.getVeterinarianCount(values);
    ...
    async loadNextPage(values: any) {
      if (this.pagination.hasMore && !this.loading) {
        await this.getVeterinarian(values, true, this.pagination.pageSize)
      }
    },
    async approve(doc_id: any) {
      this.loading = true
      const veter_doc = await getDoc(doc(fb.db, 'veterinarian_reports', doc_id))
      if (veter_doc.exists()) {
        if (veter_doc.data().decline == undefined) {
          await updateDoc(doc(fb.db, 'veterinarian_reports', doc_id), {
            approved: true,
            finished: true
          })
          this.loading = false
          this.successful += 1
          emitReportStatsUpdate(REPORT_TYPES.VETERINARIAN, 'approve', doc_id, this)
        } else if (veter_doc.data().decline != undefined) {
          await updateDoc(doc(fb.db, 'veterinarian_reports', doc_id), {
            approved: true,
            finished: true,
            decline: null
    ...
    async pending(doc_id: any) {
      this.loading = true
      await updateDoc(doc(fb.db, 'veterinarian_reports', doc_id), { approved: false })
      this.loading = false
      this.successful += 1
      emitReportStatsUpdate(REPORT_TYPES.VETERINARIAN, 'pending', doc_id, this)
    },
    async in_progress(doc_id: any) {
      ...
      this.loading = false
      this.successful += 1
      emitReportStatsUpdate(REPORT_TYPES.VETERINARIAN, 'in_progress', doc_id, this)
    },
    async decline(payload: any) {
      ...
      this.loading = false
      this.successful += 1
      emitReportStatsUpdate(REPORT_TYPES.VETERINARIAN, 'decline', doc_id, this)
    },
      ...
      try {
        for (const docId of docIds) {
          try {
            ...
            await updateDoc(doc(fb.db, 'veterinarian_reports', docId), updates)
            results.success.push(docId)

            // Emit stats update for each successful update
            emitReportStatsUpdate(REPORT_TYPES.VETERINARIAN, newStatus === 'approved' ? 'approve' : newStatus === 'in_progress' ? 'in_progress' : 'pending', docId, this)
          } catch (error) {
    ...
    async exportVeterinarian(filters: {
      category: boolean,
      state: string,
      in_progress: boolean,
      startDate?: string,
      ...
      try {
        ...
        if (filters.in_progress === true) {
          ...
          queryRef = query(collection(fb.db, 'veterinarian_reports'), ...constraints)
        ...
        console.error('Error exporting veterinarian reports:', error)
        return []
      } finally {
        this.loading = false
      }
