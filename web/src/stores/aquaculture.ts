...
export const useAquaculture = defineStore('aquaculture', {
  ...
  actions: {
    ...
    async getAquacultureCount(values: any) {
      try {
        const state = values.state
        let queryRef = collection(fb.db, 'aquaculture_reports')
        ...
        queryRef = query(collection(fb.db, 'aquaculture_reports'), ...queryConstraints)

        const snapshot = await getCountFromServer(queryRef)
        return snapshot.data().count
      } catch (error) {
        console.error('Error getting aquaculture count:', error)
        return 0
      }
    },
    ...
    async getAquaculturePage(values: any, page = 1, pageSize = 20) {
      try {
        this.loading = true;
        const state = values.state;
        let sort = false;
        ...
        if (this.pagination.pageCursors && this.pagination.pageCursors[cacheKey]) {
          this.aquaculture = this.pagination.pageCursors[cacheKey].data;
        ...
        console.error('Error fetching aquaculture page:', error);
      } finally {
        this.loading = false;
      }
    },
    ...
    async loadPagesSequentially(values: any, targetPage: number, pageSize: number, sort: boolean, state: string) {
      ...
      for (let page = 1; page <= targetPage; page++) {
        ...
        if (page === 1) {
          ...
          try {
            this.pagination.totalCount = await this.getAquacultureCount(values);
